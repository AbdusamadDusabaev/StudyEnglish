...
from text_complexity import get_text_complexity
from vocabulary_analysis import get_text_vocabulary_analysis
...
class TextAnalysisTestCase(TestCase):
 ...
 More bank failures and government bailouts can be avoided. But it will require both parties to use common sense and rediscover fiscal restraint. Let’s hope Democrats and Republicans can find a path forward.
    ...
    def test_get_result_text_complexity(self):
        result_empty_string = get_text_complexity(text=self.empty_word_string)
        result_one_word_string = get_text_complexity(text=self.one_word_string)
        result_many_word_string = get_text_complexity(text=self.many_words_string)
